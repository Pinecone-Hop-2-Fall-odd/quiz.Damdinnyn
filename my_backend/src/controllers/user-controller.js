...
export async function reqFriendId(req, res) {
    const user = req.user
    const userData = await UserModel.findOne({ _id: user.id }).populate('requestFriend')
    const reqFriend = userData.requestFriend.map((cur) => ({ username: cur.username, profile: cur.profile }))
...
export async function Passedlevels(req, res) {
    ...
    await UserModel.findByIdAndUpdate(user.id, { $push: { passedlevels: body.levelId } })
...
export async function changeProfile(req, res) {
    const body = req.body
    const user = req.user
    const userData = await UserModel.findByIdAndUpdate(user.id, { profile: body.profile })
    res.status(200).json({ userData })
}
export async function changeCollection1(req, res) {
    const user = req.user
    const body = req.body
    const userData = await UserModel.findByIdAndUpdate(user.id, { mycollection1: body.mycollection1 })
    res.status(200).json({ userData })
}
export async function changeCollection2(req, res) {
    const user = req.user
    const body = req.body
    const userData = await UserModel.findByIdAndUpdate(user.id, { mycollection2: body.mycollection2 })
    res.status(200).json({ userData })
}
...
export async function reqFriend(req, res) {
    ...
    try {
        ...
        if (reqData.requestFriend.includes(user.id)) {
            ...
            await UserModel.findByIdAndUpdate(body.toId, { $push: { requestFriend: user.id } })
            res.json({ reqData })
            console.log
        }
    }
